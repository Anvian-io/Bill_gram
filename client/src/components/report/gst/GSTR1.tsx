...
const invoiceToRows = (invoice: SalesGSTInvoice): GSTR1Row[] =>
  invoice.items.map((item: SalesGSTItem) => ({
    id: `${invoice.saleId}-${item.itemId}`,
    invoiceNo: invoice.invoiceId || "",
    salesDate: invoice.invoiceDate || "",
    partyName:
      invoice.customerName ||
      invoice.customerDetails?.companyName ||
      invoice.customerDetails?.personName ||
      "",
    stateName: getStateFromAddress(invoice.customerDetails?.address),
    gstNo: invoice.gstin || "",
    ...
    unit: item.unitName || item.unitSymbol || String(item.unit || ""),
    finalAmt: toNumber(item.finalAmount),
  }));
...
export default function GSTR1({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `gstr1-report-${timestamp}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      toast.success("GSTR1 Excel downloaded successfully");
    } catch (error) {
      console.error("Error downloading GSTR1 Excel:", error);
      toast.error("Failed to download GSTR1 Excel");
    } finally {
                ...
                <Table className={cn(layoutMode === "classic" && "classic-table", layoutMode === "classic" && "classic-table")}>
                  <TableHeader>
                    <TableRow className="bg-secondary/50">
                      ...
                      <TableHead>SALES_DATE</TableHead>
                      ...
                      {isLoading ? (
                        <motion.tr key="loading">
                          <TableCell colSpan={23} className="text-center py-12">
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <p className="text-muted-foreground">Loading GSTR1 data...</p>
                            </div>
                          </TableCell>
                        </motion.tr>
                      ) : rows.length === 0 ? (
                        ...
                        rows.map((row, index) => (
                          ...
                          >
                            ...
                            <TableCell className="text-right">{formatAmount(row.finalAmt)}</TableCell>
                          </motion.tr>
                        ))
                      )}
                    </AnimatePresence>
